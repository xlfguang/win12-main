    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (e.clientX + $("#cm")[0].offsetWidth > $("html")[0].offsetWidth) {
          $("#cm").css(
            "left",
            $("html")[0].offsetWidth - $("#cm")[0].offsetWidth - 5
          );
        }
      }, 200);
    }, 200);
...
function showdp(e, cl, arg) {
  ...
  setTimeout(() => {
    if (
      off.top + e.offsetHeight + $("#dp")[0].offsetHeight >
      $("html")[0].offsetHeight
    ) {
    ...
    if (off.left + $("#dp")[0].offsetWidth > $("html")[0].offsetWidth) {
      $("#dp").css(
        "left",
        $("html")[0].offsetWidth - $("#dp")[0].offsetWidth - 5
      );
    }
  }, 200);
}
...
function showdescp(e) {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      if (
        e.clientY + $("#descp")[0].offsetHeight + 20 >=
        $("html")[0].offsetHeight
      ) {
        $("#descp").css("top", e.clientY - $("#descp")[0].offsetHeight - 10);
      }
      if (
        e.clientX + $("#descp")[0].offsetWidth + 15 >=
        $("html")[0].offsetWidth
      ) {
...
let apps = {
  setting: {
    ...
    theme_get: () => {
      $("#set-theme")
        .html(`<loading><svg width="30px" height="30px" viewBox="0 0 16 16">
      ...
      backgroundList.forEach((fbg, i) => {
        if ($("#set-theme>loading").length) $("#set-theme").html("");
        $("#set-theme").append(
          `<a class="a act" onclick="apps.setting.theme_set('${i}')" style="background-image:url('${fbg.path}')">${fbg.name}</a>`
        );
      });
    },
    theme_set: (index) => {
      $(":root").css("--bgul", `url('${backgroundList[index].path}')`);
  ...
  whiteboard: {
    ...
    load: () => {
      ...
      apps.whiteboard.windowResizeObserver = new ResizeObserver(
        apps.whiteboard.resize
      );
    ...
    resize: () => {
      try {
        const imgData = apps.whiteboard.ctx.getImageData(
          0,
          0,
          apps.whiteboard.canvas.width,
          apps.whiteboard.canvas.height
        );
    ...
    draw: (e) => {
      let offsetX,
        offsetY,
        left = $("#win-whiteboard")[0].getBoundingClientRect().left,
  ...
  camera: {
      ...
      if (localStorage.getItem("camera")) {
        ...
        apps.camera.video.addEventListener("canplay", () => {
          if (!apps.camera.streaming) {
            apps.camera.aspectRatio =
              apps.camera.video.videoWidth / apps.camera.video.videoHeight;
            apps.camera.canvas.width = apps.camera.video.videoWidth;
            apps.camera.canvas.height = apps.camera.video.videoHeight;
            apps.camera.windowResizeObserver = new ResizeObserver(
              apps.camera.resize
            );
    ...
    takePhoto: () => {
      apps.camera.context.drawImage(
        apps.camera.video,
        0,
        0,
        apps.camera.canvas.width,
        apps.camera.canvas.height
      );
  ...
  explorer: {
    ...
    reset: () => {
      ...
      )[0].innerHTML = `<style>#win-explorer>.main>.content>.view>.class{margin-left: 20px;display: flex;}
    ...
    goto: (path) => {
      ...
      if (pathl.length == 1) {
        $("#win-explorer>.main>.content>.tool>.goback").attr(
          "onclick",
          "apps.explorer.reset()"
        );
      } else {
        $("#win-explorer>.main>.content>.tool>.goback").attr(
          "onclick",
          `apps.explorer.goto('${path.substring(
            0,
            path.length - pathl[pathl.length - 1].length - 1
          )}')`
        );
      }
    ...
    get: () => {
      $("#contri")
        .html(`<loading><svg width="30px" height="30px" viewBox="0 0 16 16">
            ...
            <circle cx="8px" cy="8px" r="7px" style="stroke:#2983cc;stroke-width:3px;"></circle></svg></loading>`);
      // 实时获取项目贡献者
      $.get("https://api.github.com/repos/tjy-gitnub/win12/contributors").then(
        (cs) => {
          setTimeout(() => {
            $("#contri").html("");
            cs.forEach((c) => {
              $("#contri").append(
                `<a class="a" onclick="window.open('${c["html_url"]}','_blank');"><p class="name">${c["login"]}</p><p class="cbs">贡献:<span class="num">${c["contributions"]}</span></p></a>`
              );
            });
            $("#contri").append(
              `<a class="button" onclick="apps.about.get()"><i class="bi bi-arrow-clockwise"></i> 刷新</a>`
            );
          }, 200);
        }
      );
  ...
  notepadFonts: {
    ...
    load: () => {
      apps.notepadFonts.fontvalues = $(
        "#win-notepad-font>.row>#win-notepad-font-type>.value-box>.option"
      );
      apps.notepadFonts.sizevalues = $(
        "#win-notepad-font>.row>#win-notepad-font-size>.value-box>.option"
      );
      apps.notepadFonts.stylevalues = $(
        "#win-notepad-font>.row>#win-notepad-font-style>.value-box>.option"
      );
      apps.notepadFonts.typeinput = $(
        "#win-notepad-font>.row>#win-notepad-font-type>input[type=text]"
      )[0];
      apps.notepadFonts.sizeinput = $(
        "#win-notepad-font>.row>#win-notepad-font-size>input[type=text]"
      )[0];
      apps.notepadFonts.styleinput = $(
        "#win-notepad-font>.row>#win-notepad-font-style>input[type=text]"
      )[0];
      apps.notepadFonts.previewBox = $(
      ...
      apps.notepadFonts.sizeinput.addEventListener(
        "keyup",
        apps.notepadFonts.preview
      );
      apps.notepadFonts.typeinput.addEventListener(
        "keyup",
        apps.notepadFonts.preview
      );
    },
    preview: () => {
      ...
      if (!apps.notepadFonts.sizeinput.value.match(/^[0-9]*$/)) {
        if (
          apps.notepadFonts.sizes[apps.notepadFonts.sizeinput.value] !=
          undefined
        ) {
      ...
      if (
        apps.notepadFonts.styles[apps.notepadFonts.styleinput.value] !=
        undefined
      ) {
        fontstyle =
          apps.notepadFonts.styles[apps.notepadFonts.styleinput.value];
      ...
      apps.notepadFonts.previewBox.attr(
        "style",
        `font-family: ${apps.notepadFonts.typeinput.value} !important; font-size: ${fontsize}px !important;${fontstyle}`
      );
    },
    commitFont: () => {
      const styles = window.getComputedStyle(
        apps.notepadFonts.previewBox[0],
        null
      );
      apps.notepadFonts.textBox.attr(
        "style",
        `font-family: ${styles.fontFamily} !important; font-size: ${styles.fontSize} !important; font-weight: ${styles.fontWeight} !important; font-style: ${styles.fontStyle} !important;`
      );
      hidewin("notepad-fonts", "configs");
    },
    reset: () => {
      const styles = window.getComputedStyle(
        apps.notepadFonts.textBox[0],
        null
      );
        ...
        if (
          ...
          try {
            ...
            if (apps.python.pyodide.runPython("sys.stdout.getvalue()")) {
              var newD = document.createElement("div");
              newD.innerText = `${apps.python.pyodide.runPython(
                "sys.stdout.getvalue()"
              )}`;
              elt.appendChild(newD);
            }
...
(c) Microsoft Corporation。all rights reserved。
    ...
    run: () => {
      ...
      if (command != "") {
        try {
          if (
            $(".window." + command)[0] &&
            !$(".window." + cmd)[0].classList.contains("configs")
          ) {
            openapp(command);
          } else {
            var newD = document.createElement("div");
  ...
  search: {
    ...
    search: (le) => {
      if (le > 0) {
        $("#search-win>.ans>.list>list").html(
          `<a class="a" onclick="apps.search.showdetail(${
            le % 8
          })"><i class="bi bi-file-earmark-${
            apps.search.rand[le % 8].bi
          }"></i> ${
            apps.search.rand[le % 8].name
          }</a><a class="a" onclick="apps.search.showdetail(${
            (le + 3) % 8
          })"><i class="bi bi-file-earmark-${
            apps.search.rand[(le + 3) % 8].bi
          }"></i> ${apps.search.rand[(le + 3) % 8].name}</a>`
        );
    ...
    showdetail: (i) => {
      ...
      $("#search-win>.ans>.view>.fname>.bi").attr(
        "class",
        "bi bi-file-earmark-" + inf.bi
      );
  ...
  edge: {
      ...
      $("#win-edge>iframe")[apps.edge.tabs.length - 1].onload = function () {
        ...
        this.contentDocument.querySelector("svg").onclick = () => {
          if ($(this.contentDocument.querySelector("input")).val() != "") {
            apps.edge.goto(
              $(this.contentDocument.querySelector("input")).val()
            );
          }
        };
      };
    ...
    moving: (t, ev, np) => {
      ...
      function move_f(e) {
        ...
        pos = Math.floor(
          (this.offsetLeft + x - deltaLeft - 36 + this.offsetWidth / 2) /
            this.offsetWidth
        );
    ...
    close: (c) => {
      $(".window.edge>.titbar>.tabs>.tab." + apps.edge.tabs[c][0]).addClass(
        "close"
      );
    ...
    tab: (c) => {
      ...
      $("#win-edge>.tool>input.url").val(
        $("#win-edge>iframe." + apps.edge.tabs[c][0]).attr("src") ==
          "mainpage.html"
          ? ""
          : $("#win-edge>iframe." + apps.edge.tabs[c][0]).attr("src")
      );
    ...
    reload: () => {
      $("#win-edge>iframe.show").attr(
        "src",
        $("#win-edge>iframe.show").attr("src")
      );
      if (
        !$(
          ".window.edge>.titbar>.tabs>.tab." +
            apps.edge.tabs[apps.edge.now][0] +
            ">.reloading"
        )[0]
      ) {
        $(
          ".window.edge>.titbar>.tabs>.tab." + apps.edge.tabs[apps.edge.now][0]
        )[0].insertAdjacentHTML("afterbegin", apps.edge.reloadElt);
        $("#win-edge>iframe." + apps.edge.tabs[apps.edge.now][0])[0].onload =
          function () {
            $(
              ".window.edge>.titbar>.tabs>.tab." + this.classList[0]
            )[0].removeChild(
              $(
                ".window.edge>.titbar>.tabs>.tab." +
                  this.classList[0] +
...
let widgets = {
  widgets: {
    add: (arg) => {
      ...
      $("#widgets>.widgets>.content>.grid")[0].innerHTML += $(
        "#widgets>.widgets>.content>.template>." + arg
      ).html();
    },
    remove: (arg) => {
  ...
  calc: {
    add: (arg) => {
      if (
        arg >= 1 &&
        arg <= 9 &&
        $("*:not(.template)>*>.wg.calc>.content>input")[0].value == "0"
      ) {
...
function pinapp(id, name, command) {
  ...
  $("#s-m-r>.pinned>.apps").append(
    `<a class='a sm-app enable ${id}' onclick='${command}';hide_startmenu();' oncontextmenu='return showcm(event,\"smapp\",[\"${id}\",\"${name}\"])'><img src='icon/${geticon(
      id
    )}'><p>${name}</p></a>`
  );
}
  ...
  if (name in icon)
    $("#taskbar").append(
      `<a class="${name}" onclick="taskbarclick(\'${name}\')" win12_title="${$(
        `.window.${name}>.titbar>p`
      ).text()}" onmouseenter="showdescp(event)" onmouseleave="hidedescp(event)" oncontextmenu="return showcm(event, 'taskbar', '${name}')"><img src="icon/${
        icon[name]
      }"></a>`
    );
  else
    $("#taskbar").append(
      `<a class="${name}" onclick="taskbarclick(\'${name}\')" win12_title="${$(
        `.window.${name}>.titbar>p`
      ).text()}" onmouseenter="showdescp(event)" onmouseleave="hidedescp(event)"><img src="icon/${name}.svg"></a>`
    );
...
function maxwin(name, trigger = true) {
  if ($(".window." + name).hasClass("max")) {
    ...
    if (
      $(".window." + name).attr("data-pos-x") != "null" &&
      $(".window." + name).attr("data-pos-y") != "null"
    ) {
      $(".window." + name).attr(
        `style`,
        `left:${$(".window." + name).attr("data-pos-x")};top:${$(
          ".window." + name
        ).attr("data-pos-y")}`
      );
    }
...
function dragBrightness(e) {
  ...
  const width = Number(
    window.getComputedStyle(container, null).width.split("px")[0]
  );
  move(e);
...
function win_move(e) {
  ...
  if (cy <= 0) {
    ...
    $(this).css(
      "cssText",
      `left:calc(100% - ${deltaLeft}px);top:${cy - deltaTop}px`
    );
    if (!(this.classList[1] in nomax)) {
...
page.addEventListener("touchend", () => {
  page.ontouchmove = null;
  if (fil) {
    if (filty == "top") maxwin(fil.classList[1], false);
    else if (filty == "left") {
      maxwin(fil.classList[1], false);
      $(fil).addClass("left");
    } else if (filty == "right") {
...
document.getElementsByTagName("body")[0].onload = function nupd() {
  ...
  document.querySelectorAll(".window").forEach((w) => {
    ...
    qw.attr(
      "onclick",
      `let os=$(this).offset();stop(event);return showcm({clientX:os.left-5,clientY:os.top+this.offsetHeight+3},'titbar','${wc}')`
    );
    qw.mousedown(stop);
...
const BASE_URL = "https://207.148.18.157:3088";
let loading = false;
